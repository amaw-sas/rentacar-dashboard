...
interface UseReservationsTableUrlStateReturn {
  ...
  onColumnFiltersChange: OnChangeFn<ColumnFiltersState>;  // react-table compat (no-op)
}
...
`FilterState` is the existing shape exported by `reservations-table.tsx`:
...
const table = useReactTable({
  data: filtered,
  columns,
  state: { sorting: url.sorting, pagination: url.pagination, columnFilters: [] },
...
- **`clearAll`**: a single `router.replace` to `pathname` with foreign params preserved. Local `searchInput` reset to empty. The pending debounce is cancelled.
...
| `tests/unit/lib/date-range.test.ts`                                        | New/Edit  | Cover `fromLocalIsoDate` and round-trip with `toLocalIsoDate`.                                                          |
...
3. **SCEN-003** — *Given* `setFilter("createdRange", { from: 2026-05-01, to: 2026-05-31 })`, *when* the URL is re-parsed, *then* the resulting `filters.createdRange` has `from.getDate() === 1`, `from.getMonth() === 4`, `to.getDate() === 31`, `to.getMonth() === 4` (local TZ).
...
6. **SCEN-006** — *Given* a URL with `?sort=created_at:desc`, *when* the hook hydrates, *then* `sorting === [PRIORITY_SORT, { id: "created_at", desc: true }]` with PRIORITY_SORT at index 0.
...
13. **SCEN-013** — *Given* filters, sort, and page are all set in the URL, *when* `clearAll()` fires, *then* exactly one `router.replace` writes `/reservations` (no query), `searchInput` resets to empty, and `filters === initialFilters`.
