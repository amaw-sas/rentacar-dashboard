...
- **Unit:** migrar `tests/unit/hooks/use-data-table-url-state.test.ts` de `replaceMock` (`vi.fn()`) a `replaceStateSpy = vi.spyOn(window.history,"replaceState").mockImplementation(()=>{})`; URL arg `[0]`→`[2]`; retener stub `useRouter` del mock `next/navigation`; `replaceMock.mockClear()`→spy; `+ afterAll(() => replaceStateSpy.mockRestore())`. **Superficie real (verificada): 46 bloques `it` (0 skip/only); tags SCEN-001..004, 006..017, 019 — SCEN-005/018 ausentes — + 4 `it` sin tag SCEN (setters/clear)** — toda referencia a `replaceMock`/`lastReplaceUrl()` migra. Atención especial a SCEN-016 (badge-click pre-debounce), SCEN-017 (no-op early-return → `NOT called`), SCEN-019 (feedback loop autoReset → `NOT called`): negativas que dependen del early-return preservado. **Gate: los 46 `it` verdes** contra el nuevo contrato. **Suite completa** verde (no solo el archivo) para descartar regresión cross-file (lección #40).
