...
- **Unit:** migrar `tests/unit/hooks/use-data-table-url-state.test.ts` de `replaceMock` (`vi.fn()`) a `replaceStateSpy = vi.spyOn(window.history,"replaceState").mockImplementation(()=>{})`; URL arg `[0]`→`[2]`; retener stub `useRouter` del mock `next/navigation`; `replaceMock.mockClear()`→spy; `+ afterAll(restore)`. Los ~24 escenarios existentes (SCEN-001..011 + setters) verdes contra el nuevo contrato. **Suite completa** verde (no solo el archivo) para descartar regresión cross-file.
...
1. **Given** un listing con `<DataTable>` (p.ej. `/customers`) cargado, **when** el operador escribe en el buscador (debounce 250ms), **then** la URL gana `?q=...`, la tabla filtra, y NO hay request RSC al segmento.
