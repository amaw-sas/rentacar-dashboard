...
to the merged log_veh bundle. Write-once after the first commit. Weakening a scenario to match output is reward
hacking — and **no scenario asserts a directional analytical result** (whether price rises toward pickup, where
the sweet spot lands) — those are data findings the report reveals, not contract.

Target: `scripts/analysis/log-veh/reports/05-anticipation.sql` (cuts 05a–05f) over the regenerated Parquet
(`search_flat.parquet` 664,126 + `cat_quotes.parquet` 2,974,126), plus its integration into the committed
markdown bundle and the PDF/Markdown presentation.

Methodology (the load-bearing invariants):
- Per-quote index `idx = total_amount / median(total_amount) PARTITION BY category_code` removes the gama
  **price-level** confound.
- The global curve is a **fixed-weight average renormalized per bucket**:
  `curve(b) = Σ_{cat∈present(b)} w_cat·median_idx(cat,b) / Σ_{cat∈present(b)} w_cat`, `w_cat` constant across
  buckets over ALL categories — removes the gama **composition** confound and never biases on missing cells.
...
- 10 ordered lead buckets: `00_0d, 01_1d, 02_2d, 03_3d, 04_4_7d, 05_8_14d, 06_15_30d, 07_31_60d, 08_61_90d,
  09_90plus`. Aggregation uses median (robust to the 95M-COP price tail).
...
**Then**: 05a has all 10 lead buckets present, each with `n_quotes`, `weighted_median_idx`, and an **integer**
`index_100`; the minimum `index_100` over confident (`n_quotes ≥ 1000`) buckets equals exactly `100`. (Pipeline
ran end-to-end; the curve's *direction* is not asserted — whatever the data says.)
...
## SCEN-002: global curve is reproducible from the all-category grid (05c)

**Given**: 05a (global curve) and 05c (per-`(category × bucket)` `median_idx` + `w_cat`, **all ~16 categories**).
**When**: for each bucket, `curve(b) = Σ_{cat present} w_cat·median_idx(cat,b) / Σ_{cat present} w_cat` is
recomputed from 05c.
**Then**: the recomputed value equals 05a's `weighted_median_idx(b)` for every bucket (within rounding). Because
`w_cat` is constant across buckets, no high-volume gama can shift the curve across lead-times.
**Evidence**: a recomputation from the 05c grid matches 05a row-for-row; 05c exposes every category code present
in `cat_quotes` (not just the top 6).

## SCEN-003: missing cells renormalize, never bias toward zero

**Given**: a lead bucket in which at least one category has zero quotes (NULL `median_idx`).
**When**: 05a computes that bucket's `weighted_median_idx`.
**Then**: the value equals the present-only renormalized weighted sum (weights of present categories summing to
1) — it is NOT dragged toward 0 by the absent category, and is NOT NULL.
**Evidence**: identify a `(bucket)` with a known-absent gama in 05c; 05a's `weighted_median_idx` for it equals
the renormalized sum over present categories and is a positive finite number.
...
## SCEN-005: reconciliation — five mutually-exclusive counts sum to the corpus
...
n_quotes_analyzed = 2,974,126` exactly, with the reasons applied in the spec's precedence (mutually exclusive).
**Evidence**: 05f's five counts sum to `2974126`; `n_quotes_analyzed` equals `Σ n_quotes` over 05a's buckets.
...
buckets / cat×buckets / weeks with `n_quotes < 1000` carry a `low_confidence` flag.
...
curve_week(06_15_30d) − 1)·100)` using the same renormalized fixed-weight method within the week.
