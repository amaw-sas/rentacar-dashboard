...
- **Proxy (vitest):** el proxy no declara vitest en sus `devDependencies`; el script `test` toma prestado `../node_modules/.bin/vitest` del root → los tests del proxy dependen del install del root (relevante para la versión de fake timers). Tests nuevos:
  ...
  - `client.test.ts` (**nuevo**) — timeout de `callLocalizaAPI`: `AbortSignal.timeout` NO es controlable con fake timers (usa un timer interno del host), así que `callLocalizaAPI` acepta un `signal` inyectable; el test inyecta un `AbortController`, registra el `fetch` mock para que **escuche el signal** (`signal.addEventListener("abort", () => reject(new DOMException("...", "AbortError")))`) y llama `controller.abort()` → lanza `LocalizaTimeoutError` (SCEN-3a). Sin fake timers.
- **Dashboard (vitest):**
