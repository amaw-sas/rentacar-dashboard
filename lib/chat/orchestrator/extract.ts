...
const SYSTEM = [
  ...
  "- ciudad vs sede: `ciudad` es la CIUDAD de recogida; `sede` es SOLO el punto/sucursal dentro de esa ciudad (ej. 'aeropuerto', 'norte', 'chipichape', 'sur'), NUNCA una ciudad. CRÍTICO: si el cliente cambia el lugar de recogida a OTRA ciudad —o nombra un punto que está en otra ciudad (típico cuando la ciudad que pidió no tiene sede y elige una cercana)— ACTUALIZA `ciudad` a esa nueva ciudad, AUNQUE ya hubiera una ciudad conocida (esto SÍ es algo nuevo, no lo dejes en null). ciudad y sede deben quedar coherentes. Ejemplo: si ya se conocía ciudad='tulua' y el cliente dice 'en palmira aeropuerto', devuelve ciudad='palmira' y sede='aeropuerto' (NUNCA dejes ciudad='tulua').",
