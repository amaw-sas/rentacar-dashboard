-- Chat Fase 2 · Incremento 2 ("cerebro"). Editable knowledge base that feeds the
-- AI chat bot as FALLBACK content (the structured tools — cotizar, info_sedes,
-- tarifa_mensual, info_gamas — are the primary source of truth; this covers what
-- they do not: policies, requirements, objections, script, tone, business model).
--
-- One markdown document per scope. V1 seeds a single shared doc (the operation is
-- Localiza-wide across the 3 brands); brand-specific overrides can be added later
-- as extra scopes. Seeded from the 363-WhatsApp-conversation consolidation
-- (lib/chat/knowledge/base-conocimiento-alquilatucarro.md). The dashboard editor
-- (/chat-knowledge) is the runtime source of truth; this seed is the initial value.
--
-- RLS mirrors the chat tables / reviews: authenticated dashboard users (all staff)
-- read and edit; the public chat route reads via the service-role admin client.
...
create policy "Authenticated users can read chat_knowledge"
  on public.chat_knowledge for select to authenticated using (true);

create policy "Authenticated users can update chat_knowledge"
  on public.chat_knowledge for update to authenticated
  using (true) with check (true);

insert into public.chat_knowledge (scope, content) values
  ('shared', $KBASE$# Base de conocimiento consolidada — AlquilaTuCarro (chatbot)
